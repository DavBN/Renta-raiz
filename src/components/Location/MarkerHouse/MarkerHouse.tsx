            ...
            <Marker key={id} position={position} icon={customIcon} eventHandlers={{
                ...
                <Popup>
                    <div className='flex items-center mb-2'>
                        <span className="mr-4 text-orange-500">
                            <FaMapMarkerAlt />
                        </span>
                        <h5 className="text-md te0xt-orange-400">{name}</h5>
                    </div>
                    <Image src={`/assets/houses/${image}`} alt={name} width={100} height={100} className="w-full h-auto" />
                    <button className="px-3 py-2 mt-2 text-white rounded-lg bg-orange-400">Ver propiedad</button>
